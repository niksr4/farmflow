        ...
        <MotionSection {...revealProps(0.05)} className="grid gap-6 lg:grid-cols-[0.92fr_1.08fr]">
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.28em] text-stone-400">Core coffee loop</p>
                <p className="mt-1 text-sm text-stone-300">From intake to saleable stock, the operating chain stays visible and explainable.</p>
              </div>
              <Badge className="w-fit border-white/10 bg-white/[0.04] text-stone-200">Live estate pattern</Badge>
            </div>

            <div className="grid gap-4 sm:grid-cols-2">
            {representativeEstateFlow.map((item, index) => {
              const Icon = item.icon
              return (
                <MotionDiv key={item.label} {...revealProps(index * 0.05)} {...hoverLift}>
                  <div className="h-full rounded-[1.6rem] border border-white/10 bg-[#0b1715] p-6 shadow-[0_18px_48px_-34px_rgba(0,0,0,0.95)]">
                    <div>
                      <div className={`flex h-11 w-11 items-center justify-center rounded-2xl border ${item.accentClassName}`}>
                        <Icon className="h-5 w-5" />
                      </div>
                      <p className="mt-5 text-2xl font-semibold text-stone-50">{item.label}</p>
                      <p className="mt-2 text-sm leading-6 text-stone-300">{item.detail}</p>
                    </div>
                    <p className="mt-6 text-3xl font-semibold text-stone-50">{item.value}</p>
                  </div>
                </MotionDiv>
              )
            })}
