...
const proofCards = [
  {
    title: "Replace the end-of-season reconciliation scramble",
    description:
      "Stock, processing, dispatch, and finance stay connected as they happen — not stitched together in a spreadsheet at the end of the month.",
    icon: Wallet,
  },
  {
    title: "Chain of custody from cherry to cash",
    description:
      "Follow every kilo from intake through dry output, confirmed dispatch, and final sale without losing the operational thread.",
    icon: PackageCheck,
  },
  {
    title: "Team-ready without the training overhead",
    description:
      "Guided setup, plain-language manuals, and role-based access get new staff recording live entries the same day.",
    icon: ShieldCheck,
  },
]
...
const representativeOpsSignals = [
  ...
  { label: "Expenses", value: "258", icon: Wallet },
]
...
const estateOutcomeCards = [
  {
    ...
    detail:
      "Track intake, conversion, dispatch, receipt, and saleable stock. Every step connected, no gaps.",
  },
  {
  ...
  },
  {
    ...
    title: "Get staff live the same day",
    detail:
      "Guided setup, safer permissions, and in-product manuals help new teams go live with fewer mistakes.",
  },
]
...
export default function LandingPage() {
  ...
  return (
    <PublicSiteShell theme="dark">
      <div className="mx-auto w-full max-w-6xl space-y-16 sm:space-y-24">
            ...
            <MotionDiv {...revealProps(0)} className="flex flex-col justify-between gap-7">
              <div className="space-y-5">
                <Badge className="w-fit border-emerald-300/20 bg-emerald-300/10 text-emerald-100 backdrop-blur">
                  For Arabica &amp; Robusta coffee estates
                </Badge>
                <div className="space-y-4">
                  ...
                  <p className="max-w-2xl text-base leading-7 text-stone-300">
                    One place to track intake, processing, labor, dispatch, and sales — so estate numbers stay clean all season, not just at close.
                  </p>
                </div>
              ...
              {/* Trust chips — replace the repeated ops-signal cards */}
              <div className="flex flex-wrap gap-2">
                {trustSignals.map(({ label, icon: Icon }) => (
                  <span
                    key={label}
                    className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs text-stone-300"
                  >
                ...
                <div className="space-y-4 p-4 sm:p-5">
                  <div className="relative h-64 overflow-hidden rounded-[1.6rem] border border-white/10 bg-[#0a1b17]">
                    ...
                    <div className="absolute inset-x-3 bottom-3 grid grid-cols-3 gap-2">
                      {representativeOpsSignals.map((signal) => {
                        ...
                        return (
                          <div key={signal.label} className="rounded-xl border border-white/10 bg-black/30 px-2.5 py-2.5 text-stone-100 backdrop-blur-md">
                            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.12em] text-stone-300">
                              <Icon className="h-3 w-3 shrink-0 text-emerald-200" />
                              <span>{signal.label}</span>
                            </div>
                            <p className="mt-1.5 text-lg font-semibold">{signal.value}</p>
                          </div>
                        )
                      })}
                    </div>
                  ...
                  <div className="rounded-[1.6rem] border border-white/10 bg-white/[0.04] p-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Badge className="border-white/10 bg-white/[0.06] text-stone-200">Representative data</Badge>
                    </div>
                  ...
                  <div className="rounded-[1.6rem] border border-amber-200/10 bg-amber-300/10 p-4">
                    ...
                    <p className="mt-2 text-lg font-semibold text-stone-50">
                      One place managers can trust the numbers that come off the estate.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <MotionSection {...revealProps(0)} className="space-y-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-medium text-emerald-200">What changes immediately</p>
              <h2 className="mt-1 font-display text-3xl font-semibold text-stone-50 sm:text-4xl">
                Estate operations that don&apos;t drift
              </h2>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-stone-300 sm:text-base">
                Most estates already know what went wrong — at the end of the season. FarmFlow surfaces the same signals daily, while there is still time to act.
              </p>
            </div>
        ...
        <MotionSection {...revealProps(0.05)} className="grid gap-6 lg:grid-cols-[0.92fr_1.08fr]">
          <div className="rounded-[1.8rem] border border-white/10 bg-[#0a1714]/90 p-6 shadow-[0_18px_54px_-34px_rgba(0,0,0,0.95)] sm:p-7">
            <p className="text-xs uppercase tracking-[0.28em] text-emerald-200">The operating surface</p>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-stone-300 sm:text-base">
              Teams need one place to understand what moved, what was sold, what is pending, and where the numbers can be trusted.
            </p>
        ...
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-medium text-emerald-200">Three plans. Start anywhere.</p>
              <h2 className="mt-1 font-display text-3xl font-semibold text-stone-50 sm:text-4xl">
                Built to fit where the estate is today
              </h2>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-stone-300 sm:text-base">
                Start with what the estate actually needs, then unlock more as operations grow. No forced upgrades — and your workspace is free to create.
              </p>
            </div>
            <Button
              variant="ghost"
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {isCore ? <span className="text-[11px] uppercase tracking-[0.24em] text-emerald-100">Most popular</span> : null}
                      </div>
        ...
        >
          ...
          <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-sm font-medium text-emerald-200">Ready when you are</p>
              <h2 className="mt-2 font-display text-3xl font-semibold text-stone-50 sm:text-4xl">
                Your estate workspace is five minutes away.
              </h2>
              <p className="mt-3 text-sm leading-7 text-stone-300 sm:text-base">
                Start recording intake, processing, labor, and dispatch from day one. Every entry connects automatically — no manual reconciliation, no spreadsheet assembly at month end.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Button
