...
Generated by `node scripts/dev/tab-seams.mjs` (re-run it; do not hand-edit the table).
...
`tenants`, `locations`, `users`, `audit_log` and `tenant_modules` are excluded — everything
touches them and their presence in a list says nothing.
...
|---|---|---|
| Revaluation counted as trade in 3 of 4 readers of the same table | `transaction_history` | ₹105.78 cr phantom depletion, ₹64.42 cr phantom purchases |
...
wrong on one side. That is the whole argument for the `seam-auditor`.
...
## The six that carry money

Start here. These are the seams where a disagreement is a rupee figure rather than a cosmetic one:
...
2. **`labour_assignments`** — Muster, Payroll, Costs, Picking, Balance Sheet. The same day's work
   priced five ways. Day-fraction totalling >1.0 lives here.
3. **`attendance_records`** — Muster, Payroll, Picking, Balance Sheet. A day marked present with
   no allocation still earns a wage in Payroll; the Muster can be silent about it.
...
6. **`booked_revenue` ↔ `sales_records` ↔ `receivables`** — revenue recognised, invoiced, and
...
mean the tab surfaces a number derived from it. Rainfall appears against seven tables because its
