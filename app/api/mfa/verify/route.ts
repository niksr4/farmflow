...
import { verifyAndEnableMfa } from "@/lib/server/mfa"
...
export async function POST(request: Request) {
  try {
    ...
    await verifyAndEnableMfa(sessionUser, token)
    ...
    return NextResponse.json({ success: false, error: error.message || "Failed to verify MFA" }, { status: 500 })
  }
}
