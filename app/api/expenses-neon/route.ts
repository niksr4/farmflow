...
import {
  buildExpenseInventoryTag,
  buildInventoryPairKey,
  buildLegacyExpenseInventoryNotes,
  isCodeTooLongError,
  isInventoryUnderflowError,
  isUnknownActivityCodeError,
  normalizeInventoryQuantity,
  parseExpenseInventoryItems,
  type PlannedExpenseInventoryTransaction,
} from "@/lib/server/expenses/inventory-notes"
import {
  buildCreateExpenseMutationStatement,
  buildDeleteExpenseInventoryLinksStatement,
  buildDeleteExpenseInventoryTransactionsStatement,
  buildDeleteExpenseStatement,
  buildInsertExpenseInventoryLinksStatement,
  buildInsertExpenseInventoryTransactionsStatement,
  buildRecalculateInventoryStatement,
  ...
  try {
    ...
    if (isInventoryUnderflowError(error)) {
      return NextResponse.json(
        {
          success: false,
          error: sanitizeRouteError(error, "Insufficient stock for the linked inventory item. Restock it first."),
        },
        { status: 409 },
      )
    }
