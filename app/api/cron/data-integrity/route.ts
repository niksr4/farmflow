...
async function handleCronInvocation(request: Request) {
  try {
    ...
    logServerError("Data integrity cron invocation failed", error)
    ...
    return NextResponse.json({ success: false, error: sanitizeRouteError(error, "Data integrity agent failed") }, { status: 500 })
  }
}
