...
// Clear this one worker's wage for the duration, so the guard is proven regardless of how the
// tenant has since been set up. Restored at the end.
...
check(res.status() === 409 && /no daily wage/i.test(bodyJson.error || ""), `refused until the work is priced: "${(bodyJson.error || "").slice(0, 60)}..."`)

console.log("\n== setting their wage unblocks it ==")
