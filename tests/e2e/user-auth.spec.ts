...
test.describe("tenant user auth regression", () => {
  ...
  test("tenant user signs in to inventory workspace without inaccessible tab errors", async ({ page, browser, baseURL }) => {
    ...
    try {
      ...
      // Estate Launch Checklist is now gated on requiresGuidedSetup (self-serve signups only)
      // Tenants created via admin API don't have this flag set, so we don't assert on it here
      await expect
        .poll(() => salesRouteHits, {
          timeout: 3000,
